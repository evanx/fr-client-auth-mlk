...
if (process.env.NODE_ENV === 'integration') {
  fastify.route({
    ...
    url: '/me',
    handler: async (request, reply) => {
      const { authorized } = request.headers
      fastify.log.debug({ authorized }, 'me')
      if (!authorized) {
        reply.code(401).send({ code: 401, message: 'Unauthorized' })
        return
      }
      const match = authorized.match(/Bearer ([^:]+):([^:]+)$/)
      if (!match) {
        reply.code(401).send({ code: 401, message: 'Invalid authorization' })
        return
      }
      const [clientReq, token] = match.slice(1)
      fastify.log.debug({ clientReq }, 'me')
      const client = await fastify.redis.hget(`session:${token}:h`, 'client')
      if (clientReq !== client) {
        reply.code(401).send({ code: 401, message: 'Invalid authorization' })
        return
      }
      reply.send({ code: 200, client })
    },
  })
}
